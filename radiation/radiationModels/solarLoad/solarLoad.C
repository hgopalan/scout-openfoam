 ...
 )
{
  ...
  volScalarField::Boundary& qrBf = qrSolar_.boundaryFieldRef();
...
bool Foam::radiation::solarLoad::updateHitFaces()
{
  if (hitFaces_.empty())
	...
	case solarCalculator::mSunDirTrackingAdv:
	  {
 ...
 )
{
  ...
  switch(solarCalc_.sunLoadModel())
    ...
    case solarCalculator::mLiuJordan:
      {
	for (const label patchID : includePatches)
	  {
	    const polyPatch& pp = patches[patchID];
...

		{
		  ...
		  if (cosEpsilon == 0.0)
		      ...
		      else
			{
		      ...
		      Ed = Y*pos(addReflected)*solarCalc_.directSolarRad();
		    }
		  else
		    {
		      //Other than vertical walls
		      scalar Klucher_F=1-sqr(solarCalc_.diffuseSolarRad()/(1e-3+solarCalc_.directSolarRad()*(solarCalc_.direction()& n[faceI])+solarCalc_.diffuseSolarRad()));
		      scalar Klucher_M3=1+Klucher_F*pow(sin(0.5*cosEpsilon),3);
		      scalar Klucher_M4=1-Klucher_F*sqr((solarCalc_.direction()& n[faceI]))*pow(sin(1.57-solarCalc_.beta()),3);
		      //Other than vertical walls
		      //Ed =solarCalc_.diffuseSolarRad()*(1.0 + cosEpsilon)/2.0*Klucher_M3*Klucher_M4;
		      Ed =solarCalc_.diffuseSolarRad()*(1.0 + cosEpsilon)/2.0;
		    }
		  Er =(pos(addReflected)*solarCalc_.directSolarRad()+solarCalc_.diffuseSolarRad())*(1.0 - cosEpsilon)/2.0;
		}
		...
		if (includeMappedPatchBasePatches[patchID])
		  {
		    for (label bandI = 0; bandI < nBands_; bandI++)
		      {
			//			qrBf[patchID][faceI] +=Ed+Er*(1-spectralDistribution_[bandI]*absorptivity_[patchID][bandI]()[faceI]);
			//qrBf[patchID][faceI] +=(Ed + Er)*spectralDistribution_[bandI]*absorptivity_[patchID][bandI]()[faceI];
			qrBf[patchID][faceI] +=Ed+0*Er*(1-spectralDistribution_[bandI]*absorptivity_[patchID][bandI]()[faceI]);
		      }
		  }
		else
		  {
...
void Foam::radiation::solarLoad::initialise(const dictionary& coeffs)
{
  ...
  coeffs.readIfPresent("noTrees",noTrees_);
  ...
  if (facesChanged)
    {
      ...
      // 2/1/2020 - Turn off direct radiation if trees are included 
      // Direct radiation calculated differently 
      if(noTrees_)
	{
