...
void Foam::radiation::fvDOM::initialise()
{
  ...
  if (useSolarLoad_)
	...
	<< abort(FatalError);
      ...
      const dictionary& solarDict1 = this->subDict("solarLoadCoeffs");
      solarLoad_.reset(new solarLoad(solarDict1, T_));
...
void Foam::radiation::fvDOM::updateG()
{
  ...
  qcb_ = dimensionedScalar(dimMass/pow3(dimTime), Zero);
  ...
  //  volVectorField meshC=mesh_.C()/(mag(mesh_.C())+1e-5);
  vector meshC(0,0,1);
  qmrtlw_=0*G_;
  qmrtsw_=0*G_;
  forAll(IRay_, rayI)
    {
      ...
      if(rayI==SunRayId)
	{
	  qcb_= IRay_[rayI].I()*IRay_[rayI].omega();
	  qmrtsw_=IRay_[rayI].I()*IRay_[rayI].omega()*mag(IRay_[rayI].d() & meshC);	  
	}
      else
	qmrtlw_+=IRay_[rayI].I()*IRay_[rayI].omega()*(IRay_[rayI].d() & meshC);	
      ...
      qcb_.boundaryFieldRef() += IRay_[rayI].qcb().boundaryField();
    }
}
